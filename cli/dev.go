...
func NewDevCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, v := range strings.Split(services, ",") {
			...
			latestDev := store.Latest(state.ReleaseKindDev).Copy()
			...
			if from != "" {
				...
				copied, err := store.GetRelease(hash)
				if err != nil {
				...
				copied.Tag = nextTag
				copied.Kind = state.ReleaseKindDev
				if err := store.CreateRelease(copied); err != nil {
	...
	cmd.Flags().StringVarP(&services, "services", "s", "",
		"Comma separated list of services and version to use. (ex: service1@v1,service2@v2).",
	)
	return cmd
}
